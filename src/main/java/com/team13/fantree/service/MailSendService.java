...
import com.team13.fantree.jwt.RedisUtil;
...
public class MailSendService {
    ...
    @Transactional
    public void CheckAuthNum(long userId, String email, String authNum) {
        User user = userRepository.findById(userId).orElseThrow(
                () -> new NotFoundException(UserErrorCode.USER_NOT_FOUND)
        );
        log.info("Check auth num: " + authNum);
        log.info("Check email: " + email);

        if (!redisUtil.getData(authNum).equals(email)) {
            throw new MismatchException(UserErrorCode.AUTH_MISMATCH);
        }
    ...
    public String joinEmail(String email) {
        makeRandomNumber();
        String setFrom = "clsrn77990@gmail.com"; // email-config에 설정한 자신의 이메일 주소를 입력
        String toMail = email;
